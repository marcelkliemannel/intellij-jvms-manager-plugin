...
class FindProcessAction : DumbAwareAction("Find Process Information", null, AllIcons.Actions.Find) {
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    val pid = Messages.showInputDialog(e.project, "Please enter a PID:", "Find Process Information",
                                       null, null, IntInputValidator.INSTANCE)?.toIntOrNull() ?: return

    val onSuccess: (ProcessNode?) -> Unit = {
      if (it != null) {
        e.project?.getService(JpsPluginService::class.java)?.showProcessDetails(it)
      }
      else {
        Messages.showErrorDialog(e.project, "No process with PID $pid found.", "Finding Process Information Failed")
      }
    }
    ...
    val onThrowable: (Throwable) -> Unit = { error ->
      ...
      Messages.showErrorDialog(e.project, "$errorMessage\n\nSee idea.log for more details.", "Finding Process Information Failed")
    }

    findProcessNodeTaskRunning= true
    FindProcessNodeTask(pid, e.project, onSuccess, { findProcessNodeTaskRunning = false }, onThrowable).queue()
  }
