...
class AttachJavaAgentJvmAction : JvmAction("Attach Java Agent") {
  ...
  override fun createComponent(project: Project, parent: Disposable) =
    JPanel(GridBagLayout()).apply {
      ...
      val agents =
        listOf(
          InstrumentationAgentReference(project, parent),
      ...
      add(
        agentReferenceLabel,
        bag.nextLine().next().anchor(GridBagConstraints.WEST).overrideTopInset(UIUtil.DEFAULT_VGAP),
      )
      add(
        agentReferenceWrapper,
        bag
          .next()
      ...
      add(
        ...
        bag
          ...
          .weightx(1.0)
          .fillCellHorizontally()
          .overrideTopInset(UIUtil.DEFAULT_VGAP / 2),
      )

      add(
        JBLabel("Options:"),
        bag.nextLine().next().overrideTopInset(UIUtil.DEFAULT_VGAP).anchor(GridBagConstraints.WEST),
      )
      add(
        optionsField,
        bag
      ...
      add(
        JButton().apply { action = createAttachJavaAgentAction(this) },
  ...
  private fun updateAgentReference() {
    ...
    agentReferenceHelpLabel.text =
      "<html><a href=\"${selectedAgentType.agentReferenceHelpLink}\">${selectedAgentType.agentReferenceHelpTitle}</a></html>"
  }

  private fun getSelectedAgentType(): AgentReference =
    agentReferenceSelection.selectedItem as AgentReference
  ...
  private fun createAttachJavaAgentAction(parentComponent: JComponent): Action {
    return object : AbstractAction("Attach", AllIcons.RunConfigurations.TestState.Run) {
      override fun actionPerformed(e: ActionEvent) {
        val jvmActionContext =
          CommonsDataKeys.getRequiredData(
            JvmActionContext.DATA_KEY,
        ...
        AttachAgentTask(jvmActionContext, selectedAgentType, options) {
            parentComponent.isEnabled = true
          }
          .queue()
      }
    }
  ...
  private class AttachAgentTask(
    ...
    override fun run(indicator: ProgressIndicator) {
      ...
      LOG.info(
        "Attaching Java agent '$agentReference' (${this.agentReference.name}) ${if (options != null) " with options $options" else ""} " +
          "to process with PID ${jvmActionContext.processNode.process.processID}..."
      )
      (jvmActionContext.processNode as JvmProcessNode).attachJavaAgent(
        agentReference.javaAgentType,
        agentReferenceValue,
        options,
      )
    }
    ...
    override fun onSuccess() {
      NotificationUtils.notifyOnToolWindow(
        "Java agent attached to process with PID ${jvmActionContext.processNode.process.processID}.",
        project,
      )
    }
  ...
  private abstract class AgentReference(
    val name: String,
    ...
    companion object {
      ...
      fun createAgentPathFieldValidator(field: JTextField, parent: Disposable): ComponentValidator {
        return ComponentValidator(parent)
          .withValidator(
            Supplier {
              when {
                field.text.isBlank() -> {
  ...
  private class InstrumentationAgentReference(project: Project, parent: Disposable) :
    ...
    companion object {
      ...
      fun createAgentJarPathField(
        project: Project,
        parent: Disposable,
      ): Pair<JTextField, ComponentValidator> {
        val agentPathField = TextFieldWithBrowseButton()
        val fileDescriptor =
          FileChooserDescriptor(true, false, true, true, false, false).withFileFilter { file ->
            Comparing.equal(file.extension, "jar", SystemInfo.isFileSystemCaseSensitive)
          }
        agentPathField.addBrowseFolderListener(
          "Java Agent File",
          "Select the Java agent JAR file.",
          project,
          fileDescriptor,
        )

        val agentPathFieldValidator =
          createAgentPathFieldValidator(agentPathField.textField, parent)
  ...
  private class LibraryPathAgentReference(project: Project, parent: Disposable) :
    ...
    companion object {
      ...
      fun createAgentLibraryPathField(
        project: Project,
        parent: Disposable,
      ): Pair<JTextField, ComponentValidator> {
        val agentPathField = TextFieldWithBrowseButton()
        ...
        agentPathField.addBrowseFolderListener(
          "Java Agent Library File",
          "Select the Java agent library file.",
          project,
          fileDescriptor,
        )
      ...
      fun createAgentLibraryPathField(parent: Disposable): Pair<JTextField, ComponentValidator> {
        ...
        val agentNameFieldValidator =
          ComponentValidator(parent)
            ...
            .andRegisterOnDocumentListener(agentNameField)
            .installOn(agentNameField)
