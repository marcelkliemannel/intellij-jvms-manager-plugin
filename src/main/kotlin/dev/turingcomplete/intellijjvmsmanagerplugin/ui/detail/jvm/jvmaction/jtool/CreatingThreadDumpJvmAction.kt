...
class CreatingThreadDumpJvmAction : JvmAction("Thread Dump") {
  ...
  private fun createOpenResultInThreadDumpAnalyzerRunOption(): AnAction {
    val onSuccess: (String, JvmActionContext) -> Unit = { threadDump, jvmActionContext ->
      ...
      val threadDumpConsoleFactory = if (threadStates.size > 1) ThreadDumpConsoleFactory(project, threadStates) else null
      val tabTitle = "${jvmActionContext.processNode.smartName} (${jvmActionContext.processNode.process.processID})"
      val icon = AllIcons.Actions.Dump
      AnalyzeStacktraceUtil.addConsole(project, threadDumpConsoleFactory, tabTitle, threadDump, icon)
    }
    return ToStringJToolRunOption("Run - Show In Thread Dump Analyzer", toTaskTitleFunction, createJToolCommand, onSuccess)
  }
  ...
  private fun createOpenResultInPopupRunOption(): AnAction {
    ...
    return ToStringJToolRunOption("Run - Show Output", toTaskTitleFunction, createJToolCommand, onSuccess)
  }
