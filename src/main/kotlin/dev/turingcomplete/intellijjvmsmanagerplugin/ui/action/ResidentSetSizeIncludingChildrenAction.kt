...
class ResidentSetSizeIncludingChildrenAction : DumbAwareAction() {
  ...
  override fun update(e: AnActionEvent) {
    ...
    if (processes.size == 1 && processes[0].childCount > 0) {
      ...
      e.presentation.text =
        "Resident Set Size (RSS) including children: ${StringUtil.formatFileSize(sumUpResidentSetSize(processes[0]))}"
    } else {
  ...
  private fun sumUpResidentSetSize(processNode: ProcessNode): Long {
    return processNode.process.residentSetSize +
      processNode.children().asSequence().sumOf { sumUpResidentSetSize(it as ProcessNode) }
  }
